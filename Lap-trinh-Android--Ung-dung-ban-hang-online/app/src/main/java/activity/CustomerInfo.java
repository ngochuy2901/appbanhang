...
public class CustomerInfo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(CheckInternetConnection.haveNetworkConnection(getApplicationContext())){
            ...
            CheckInternetConnection.ShowToast_Short(getApplicationContext(),"Kiểm tra kết nối");
        }
    }
        ...
        btnConfirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(Name.length()>0&& Phone.length()>0&&email.length()>0){
                    ...
                    StringRequest stringRequest = new StringRequest(Request.Method.POST, Server.billlink, new Response.Listener<String>() {
                        @Override
                        public void onResponse(final String madonhang) {
                            Log.d("Ma don hang",madonhang);
                            if(Integer.parseInt(madonhang)>0){
                                RequestQueue queue = Volley.newRequestQueue(getApplicationContext());
                                StringRequest request = new StringRequest(Request.Method.POST, Server.orderdetaillink, new Response.Listener<String>() {
                                    @Override
                                    public void onResponse(String response) {
                                        if(response.equals("1")) {
                                            MainActivity.arrCart.clear();
                                            CheckInternetConnection.ShowToast_Short(getApplicationContext(),"Đặt hàng thành công!");
                                            Intent intent = new Intent(getApplicationContext(),MainActivity.class);
                                            startActivity(intent);
                                            CheckInternetConnection.ShowToast_Short(getApplicationContext(),"Tiếp tục");
                                        }else{
                                            CheckInternetConnection.ShowToast_Short(getApplicationContext(),"Dữ liệu bị lỗi!");
                                        }
                                    }
                    ...
                    CheckInternetConnection.ShowToast_Short(getApplicationContext(),"Kiểm tra dữ liệu");
                }
            }
        });
    }
